...
@Component
public class VltEwmaVol32SlpCalc implements DescribableCalc {
    ...
    public static double calculate(BarSeries series, int index) {
        ...
        for (int i = start; i <= index; i++) { double x = k++; double y = ewmaVol(series, i, 32); sumX += x; sumY += y; sumXY += x*y; sumX2 += x*x; }
        double denom = n * sumX2 - sumX * sumX;
        return (Math.abs(denom) < 1e-12) ? 0 : (n * sumXY - sumX * sumY) / denom;
    }
    @Override public FeatureDescription describe() { return new FeatureDescription("vlt_ewma_vol_32_slp", "EWMA Vol 32 Slope", "volatility", "slope(ewma, 20)", "Slope EWMA vol.", "unbounded", ""); }
}
