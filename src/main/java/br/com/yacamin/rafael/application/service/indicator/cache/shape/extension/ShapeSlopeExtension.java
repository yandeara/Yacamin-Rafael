...
/**
 * Slope (regressao linear OLS) generico para indicadores de shape sobre uma janela.
 * beta = (n*sum(x*y) - sum(x)*sum(y)) / (n*sum(x^2) - sum(x)^2)
 */
public class ShapeSlopeExtension extends CachedIndicator<Num> {
    ...
    public ShapeSlopeExtension(Indicator<Num> source, int window) {
