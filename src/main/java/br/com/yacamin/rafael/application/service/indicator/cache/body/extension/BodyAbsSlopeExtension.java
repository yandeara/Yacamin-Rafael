...
/**
 * Slope (regressao linear OLS) do body absoluto sobre uma janela.
 * beta = (n*sum(x*y) - sum(x)*sum(y)) / (n*sum(x^2) - sum(x)^2)
 */
public class BodyAbsSlopeExtension extends CachedIndicator<Num> {
    ...
    public BodyAbsSlopeExtension(Indicator<Num> source, int window) {
