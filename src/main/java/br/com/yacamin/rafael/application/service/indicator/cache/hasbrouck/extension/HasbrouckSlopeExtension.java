...
/**
 * Slope (regressao linear) do Hasbrouck Lambda sobre uma janela.
 * Calcula o coeficiente angular (beta) de OLS: y = source values, x = 0..window-1
 * beta = (n*sum(x*y) - sum(x)*sum(y)) / (n*sum(x^2) - sum(x)^2)
 */
public class HasbrouckSlopeExtension extends CachedIndicator<Num> {

    private final Indicator<Num> source;
    private final int window;

    public HasbrouckSlopeExtension(Indicator<Num> source, int window) {
