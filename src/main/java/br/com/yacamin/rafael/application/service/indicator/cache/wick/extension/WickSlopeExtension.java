...
/**
 * Slope (regressao linear OLS) generico sobre uma janela.
 * beta = (n*sum(x*y) - sum(x)*sum(y)) / (n*sum(x^2) - sum(x)^2)
 */
public class WickSlopeExtension extends CachedIndicator<Num> {
    ...
    public WickSlopeExtension(Indicator<Num> source, int window) {
