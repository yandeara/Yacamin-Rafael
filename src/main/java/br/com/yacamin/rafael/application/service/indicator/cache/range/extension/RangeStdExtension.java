...
/**
 * Desvio padrao sobre uma janela de qualquer source (Range, HLC3, LogRange, etc.).
 * STD(source, window) = sqrt(sum((x - mean)^2) / window)
 */
public class RangeStdExtension extends CachedIndicator<Num> {

    private final Indicator<Num> source;
    private final int window;

    public RangeStdExtension(Indicator<Num> source, int window) {
