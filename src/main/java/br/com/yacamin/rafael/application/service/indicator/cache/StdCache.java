...
@Component
public class StdCache implements IndicatorCache {

    private final Map<String, StandardDeviationIndicator> cache = new ConcurrentHashMap<>();
    private final CloseCache closeCache;

    public StdCache(CloseCache closeCache) {
        this.closeCache = closeCache;
    }

    public StandardDeviationIndicator getStd(String symbol, CandleIntervals interval, BarSeries series, int period) {
        return cache.computeIfAbsent(symbol + "_" + interval.name() + "_" + period, k -> {
            var close = closeCache.getClosePrice(symbol, interval, series);
            return new StandardDeviationIndicator(close, period);
        });
    }
