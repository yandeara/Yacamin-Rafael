...
/**
 * Slope (regressao linear OLS) do body ratio sobre uma janela.
 * beta = (n*sum(x*y) - sum(x)*sum(y)) / (n*sum(x^2) - sum(x)^2)
 */
public class BodyRatioSlopeExtension extends CachedIndicator<Num> {
    ...
    public BodyRatioSlopeExtension(Indicator<Num> source, int window) {
